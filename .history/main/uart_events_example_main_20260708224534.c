...
static esp_err_t send_at_cmd(uart_port_t uart, const char *cmd, uint32_t wait_ms)
{
    ...
    recv_len = uart_read_bytes(uart, recv_buf, sizeof(recv_buf) - 1, pdMS_TO_TICKS(200));
...
// ===================== 任务3：模块AT初始化配置（最高优先级，先锁串口再延时） =====================
// 封装带重试发送函数
static esp_err_t send_at_retry(uart_port_t uart, const char *cmd, uint32_t wait_ms, uint8_t retry_max)
{
    esp_err_t ret;
    for (uint8_t i = 0; i < retry_max; i++)
    {
        ret = send_at_cmd(uart, cmd, wait_ms);
        if (ret == ESP_OK)
            return ESP_OK;
        ESP_LOGW(TAG, "Retry %d/%d for cmd %s", i + 1, retry_max, cmd);
        vTaskDelay(pdMS_TO_TICKS(400));
    }
    ESP_LOGE(TAG, "Cmd %s all retry failed!", cmd);
    return ESP_FAIL;
}
...
static void lora_module_at_config_task(void *pvParameters)
{
    ...
    send_at_retry(LORA_TX_UART, "+++", 600, 3);
    send_at_retry(LORA_TX_UART, "AT+MODE1\r\n", 800, 3);
    send_at_retry(LORA_TX_UART, "AT+LEVEL3\r\n", 800, 3);
    // 先不发RESET，避免OK被重启截断，验证AT交互正常后再加
    // send_at_retry(LORA_TX_UART, "AT+RESET\r\n", 1000, 3);
    vTaskDelay(pdMS_TO_TICKS(1500));

    ESP_LOGI(TAG, "==== Config RX LoRa Module ====");
    send_at_retry(LORA_RX_UART, "+++", 600, 3);
    send_at_retry(LORA_RX_UART, "AT+MODE1\r\n", 800, 3);
    send_at_retry(LORA_RX_UART, "AT+LEVEL3\r\n", 800, 3);
