...
static void lora_uart_init(uart_port_t uart_num, gpio_num_t tx_pin, gpio_num_t rx_pin, QueueHandle_t *evt_queue)
{
    uart_config_t uart_cfg = {
        .baud_rate = 9600, // 修正：原115200改为模块默认9600
...
static esp_err_t send_at_cmd(uart_port_t uart, const char *cmd, uint32_t wait_ms)
{
    ...
    size_t recv_len;
    // 仅清空接收缓冲区，ESP-IDF无uart_flush_output API
    uart_flush_input(uart);

    if (strcmp(cmd, "+++") == 0)
    {
        // DX-LR41手册强制：进入AT模式前后必须≥1ms静默
...
{
    // 第一步：立刻独占UART2互斥锁，全程阻断定时发包任务
    xSemaphoreTake(uart2_mutex, portMAX_DELAY);
    // 持有锁等待模块上电稳定，总线全程干净无业务数据包
    ...
    vTaskDelay(pdMS_TO_TICKS(1200)); // 等待模块重启生效

    // ========== 配置接收模块 UART1（独立串口，无需uart2_mutex） ==========
